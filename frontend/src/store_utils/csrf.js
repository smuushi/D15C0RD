
export async function csrfFetch(url, options = {}) {
    // set options.method to 'GET' if there is no method
    options.method = options.method || 'GET';
    // set options.headers to an empty object if there are no headers
    options.headers = options.headers || {};
  
    // if the options.method is not 'GET', then set the "Content-Type" header to
    // "application/json" and the "X-CSRF-Token" header to the value of the 
    // "X-CSRF-Token" cookie
    if (options.method.toUpperCase() !== 'GET') {
        if (!options.headers["Content-Type"] && !(options.body instanceof FormData)) {
            options.headers['Content-Type'] =
            options.headers['Content-Type'] || 'application/json';
        }
        options.headers['X-CSRF-Token'] = sessionStorage.getItem('X-CSRF-Token');
    }
  
    // call fetch with the url and the updated options hash

        const res = await fetch(url, options).catch(async (res) => {
            let errors = await res.json() 
            // console.log(errors)
        })


        return res
    
  
    // if the response status code is 400 or above, then throw an error with the
    // error being the response
    // if (res.status >= 400) throw res;
  
    // if the response status code is under 400, then return the response to the
    // next promise chain
    // return res
  }
  

export function storeCSRFToken(response) {
    const csrfToken = response.headers.get("X-CSRF-Token");

    // debugger
    // console.log(csrfToken)
    if (csrfToken) {
        // setTimeout(() => {
        //     // console.log('attempting to set item')


        
        sessionStorage.setItem('X-CSRF-Token', csrfToken)

        // while (!sessionStorage.getItem('X-CSRF-Token')){
        //     // console.log("help")
        //     sessionStorage.setItem('X-CSRF-Token', csrfToken)
        // }
        // }, 2000);
    }
}

const storeSessionStorageUser = async (res) => {
    if (res.status === 209) {
        //IT'S NULL SO NO USERS 
        sessionStorage.setItem('currentUser', "null")
        
    } else if (res.status === 200) {
        let data = await res.json()
        sessionStorage.setItem('currentUser', JSON.stringify(data))
    }
        // console.log("set sessionStorage.. not the store.")
}

export async function restoreSessionStorage() {
    const response = await csrfFetch("/api/session");
    // // console.log(response)
    // debugger

    storeCSRFToken(response);

    storeSessionStorageUser(response)
    
    return response;
}

// the above should only be called when reloading the page and something is missing in the
// browser's session storage..